function Promotion10() {}

Promotion10.prototype.Schema =
	"<element name='Entity'>" +
		"<text/>" +
	"</element>" +
...
Promotion10.prototype.Init = function()
{
	this.currentXp = 0;
};

Promotion10.prototype.GetRequiredXp = function()
{
	return ApplyValueModificationsToEntity("Promotion10/RequiredXp", +this.template.RequiredXp, this.entity);
};

Promotion10.prototype.GetCurrentXp = function()
{
	return this.currentXp;
};

Promotion10.prototype.GetPromotedTemplateName = function()
{
	return this.template.Entity;
};

Promotion10.prototype.Promote = function(promotedTemplateName)
{
...
Promotion10.prototype.IncreaseXp = function(amount)
{
	...
	{
		let cmpPromotion10 = Engine.QueryInterface(this.promotedUnitEntity, IID_Promotion10);
		if (cmpPromotion10)
			cmpPromotion10.IncreaseXp(amount);
		return;
	}
	...
	if (this.currentXp >= requiredXp)
	{
		...
		while (true)
		{
			...
			if (!template.Promotion10)
				break;
			requiredXp = ApplyValueModificationsToTemplate("Promotion10/RequiredXp", +template.Promotion10.RequiredXp, playerID, template);
			// compare the current xp to the required xp of the promoted entity
			if (this.currentXp < requiredXp)
				break;
			this.currentXp -= requiredXp;
			promotedTemplateName = template.Promotion10.Entity;
		}
		this.Promote(promotedTemplateName);
		let cmpPromotion10 = Engine.QueryInterface(this.promotedUnitEntity, IID_Promotion10);
		if (cmpPromotion10)
			cmpPromotion10.IncreaseXp(this.currentXp);
	}

	Engine.PostMessage(this.entity, MT_ExperienceChanged, {});
};

Promotion10.prototype.OnValueModification = function(msg)
{
	if (msg.component == "Promotion10")
		this.IncreaseXp(0);
};

Engine.RegisterComponentType(IID_Promotion10, "Promotion10", Promotion10);
