function Promotion16() {}

Promotion16.prototype.Schema =
	"<element name='Entity'>" +
		"<text/>" +
	"</element>" +
...
Promotion16.prototype.Init = function()
{
	this.currentXp = 0;
};

Promotion16.prototype.GetRequiredXp = function()
{
	return ApplyValueModificationsToEntity("Promotion16/RequiredXp", +this.template.RequiredXp, this.entity);
};

Promotion16.prototype.GetCurrentXp = function()
{
	return this.currentXp;
};

Promotion16.prototype.GetPromotedTemplateName = function()
{
	return this.template.Entity;
};

Promotion16.prototype.Promote = function(promotedTemplateName)
{
...
Promotion16.prototype.IncreaseXp = function(amount)
{
	...
	{
		let cmpPromotion16 = Engine.QueryInterface(this.promotedUnitEntity, IID_Promotion16);
		if (cmpPromotion16)
			cmpPromotion16.IncreaseXp(amount);
		return;
	}
	...
	if (this.currentXp >= requiredXp)
	{
		...
		while (true)
		{
			...
			if (!template.Promotion16)
				break;
			requiredXp = ApplyValueModificationsToTemplate("Promotion16/RequiredXp", +template.Promotion16.RequiredXp, playerID, template);
			// compare the current xp to the required xp of the promoted entity
			if (this.currentXp < requiredXp)
				break;
			this.currentXp -= requiredXp;
			promotedTemplateName = template.Promotion16.Entity;
		}
		this.Promote(promotedTemplateName);
		let cmpPromotion16 = Engine.QueryInterface(this.promotedUnitEntity, IID_Promotion16);
		if (cmpPromotion16)
			cmpPromotion16.IncreaseXp(this.currentXp);
	}

	Engine.PostMessage(this.entity, MT_ExperienceChanged, {});
};

Promotion16.prototype.OnValueModification = function(msg)
{
	if (msg.component == "Promotion16")
		this.IncreaseXp(0);
};

Engine.RegisterComponentType(IID_Promotion16, "Promotion16", Promotion16);
