...
Promotion5.prototype.Init = function()
{
	this.currentXp = 0;
};

Promotion5.prototype.GetRequiredXp = function()
{
	return ApplyValueModificationsToEntity("Promotion5/RequiredXp", +this.template.RequiredXp, this.entity);
};

Promotion5.prototype.GetCurrentXp = function()
{
	return this.currentXp;
};

Promotion5.prototype.GetPromotedTemplateName = function()
{
	return this.template.Entity;
};

Promotion5.prototype.Promote = function(promotedTemplateName)
{
...
Promotion5.prototype.IncreaseXp = function(amount)
{
	...
	{
		let cmpPromotion5 = Engine.QueryInterface(this.promotedUnitEntity, IID_Promotion5);
		if (cmpPromotion5)
			cmpPromotion5.IncreaseXp(amount);
		return;
	}
	...
	if (this.currentXp >= requiredXp)
	{
		...
		while (true)
		{
			...
			if (!template.Promotion5)
				break;
			requiredXp = ApplyValueModificationsToTemplate("Promotion5/RequiredXp", +template.Promotion5.RequiredXp, playerID, template);
			// compare the current xp to the required xp of the promoted entity
			if (this.currentXp < requiredXp)
				break;
			this.currentXp -= requiredXp;
			promotedTemplateName = template.Promotion5.Entity;
		}
		this.Promote(promotedTemplateName);
		let cmpPromotion5 = Engine.QueryInterface(this.promotedUnitEntity, IID_Promotion5);
		if (cmpPromotion5)
			cmpPromotion5.IncreaseXp(this.currentXp);
	}

	Engine.PostMessage(this.entity, MT_ExperienceChanged, {});
};

Promotion5.prototype.OnValueModification = function(msg)
{
	if (msg.component == "Promotion5")
		this.IncreaseXp(0);
};

Engine.RegisterComponentType(IID_Promotion5, "Promotion5", Promotion5);
